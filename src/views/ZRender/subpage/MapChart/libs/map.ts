...
import ZRenderLayer from './ZRenderLayer'
...
export class LMap {
  private map!: L.Map
  private zRenderLayer!: ZRenderLayer
  ...
  private initZRenderLayer() {
    this.zRenderLayer = new ZRenderLayer(this, this.map, { zIndex: 1000 })
    this.addZrLayers()
    return this
  }
  private addZrLayers() {
    if (this.options.area?.enable) this.initArea()
  }
  private initArea() {
    const area = new Area({
      itemStyleActive: {
