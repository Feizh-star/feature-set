...
export function useGlStainImg({ mapIns }: { mapIns: ShallowRef<L.Map | null> }) {
  ...
  const addGlStainImg = (url: string, color: IHclColorsRange, scale: IScaleProps, options: { [p: string]: any }) => addGlStainImgFunc(mapIns, glStainImgLayer, url, color, scale, options)
  ...
  const getValueByPosition = (lat: number, lon: number) => getValueByPositionFunc(glStainImgLayer, lat, lon)
...
function getValueByPositionFunc(glStainImgLayer: ShallowRef<TGlStainImgLayer>, lat: number, lon: number) {
 ...
 * 移除色斑图
 */
function removeGlStainImgFunc(mapIns: ShallowRef<L.Map | null>, glStainImgLayer: ShallowRef<TGlStainImgLayer>) {
  if (!glStainImgLayer.value) return
  if (mapIns.value) {
