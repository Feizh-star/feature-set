...
export function useGeoJsonLine({ mapIns }: { mapIns: ShallowRef<L.Map | null> }) {
  ...
  const addGeoJsonLine = (layers: Array<TAnyObject>, cfg?: TAnyObject) => addGeoJsonLineFunc(mapIns, geojsonLineLayer, layers, cfg)
  const removeGeoJsonLine = (layerName: string) => removeGeoJsonLineFunc(geojsonLineLayer, layerName)
  onBeforeUnmount(() => {
...
function removeGeoJsonLineFunc(geojsonLineLayer: ShallowRef<TGeojsonLinesLayer>, layerName: string) {
  if (!geojsonLineLayer.value) return
  geojsonLineLayer.value.removeData(layerName)
}

function removeGeoJsonLineInstanceFunc(mapIns: ShallowRef<L.Map | null>, geojsonLineLayer: ShallowRef<TGeojsonLinesLayer>) {
  if (!geojsonLineLayer.value) return
  if (mapIns.value) {
