...
export function useMarkers({ mapIns }: { mapIns: ShallowRef<L.Map | null> }) {
  ...
  const addMarkers = (name: string, markers: IMarkerItem[], commonOption: ICommonMarkerOption = {}, options: L.MarkerOptions = {}) => 
 ...
 * @param name 名称
 */
function removeMarkersFunc(mapIns: ShallowRef<L.Map | null>, markerMap: Map<string, IMarkerGroup>, name: string) {
 ...
 * @returns void
 */
function handleMapEventsForMarkersFunc(mapIns: ShallowRef<L.Map | null>, markerMap: Map<string, IMarkerGroup>, name: string) {
  if (!mapIns.value) return
  const markerGroup = markerMap.get(name)
 ...
 * @returns void
 */
function removeMapEventsForMarkersFunc(mapIns: ShallowRef<L.Map | null>, markerMap: Map<string, IMarkerGroup>, name: string) {
  if (!mapIns.value) return
  const markerGroup = markerMap.get(name)
  if (!markerGroup) return
