...
const innerValue = computed({
  get() {
    ...
    if (innerRange.value) {
      // 范围选择有个一特殊情况：逐旬 逐季时需要对进来的日期进行范围格式化
...
function parseInnerValueToRightFormat(currentDateString: string, type: TypeString) {
  const { 'value-type': valueType } = attrs
  let result
  switch (type) {
    case 'ten':
      // 将时间转换为 当前旬时间段
      result = getTenRangeByDate(new Date(currentDateString)).map(d => dateToString(d, valueType as string))
      break
    case 'quarter':
      // 将时间转换为 当前季度时间段
      result = getQuarterRangeByDate(new Date(currentDateString)).map(d => dateToString(d, valueType as string))
      break
    case 'week':
    case 'month':
    case 'year':
...
function packageStringDate(pickerValue: string, type: TypeString = props.type) {
  ...
  switch (type) {
    ...
    case 'week':
      ...
      result = getWeekRange(pickerValue, valueType as string)
      break
    case 'month':
      result = getMonthRange(pickerValue, valueType as string)
      break
    case 'year':
      result = getYearRange(pickerValue, valueType as string)
      break
    default:
      result = [pickerValue]
  }
...
function getWeekRange(pickerValue: string, parseFormat: string) {
  ...
  if (disableStart) {
    weekEnd = new Date(weekEnd).getTime() - disableStart.getTime() > 0 ? dateToString(disableStart, parseFormat) : weekEnd
  }
...
function getMonthRange(pickerValue: string, parseFormat: string) {
  ...
  let monthEnd = dateToString(new Date(new Date(addHours(monthStartDateString, (monthDays - 1) * 24, parseFormat)).setHours(23, 59, 59, 999)), parseFormat)
  const { disableStart } = props
  if (disableStart) {
    monthEnd = new Date(monthEnd).getTime() - disableStart.getTime() > 0 ? dateToString(disableStart, parseFormat) : monthEnd
  }
...
function getYearRange(pickerValue: string, parseFormat: string) {
  ...
  let yearEnd = dateToString(new Date(new Date(addHours(yearStartDateString, (yearDays - 1) * 24, parseFormat)).setHours(23, 59, 59, 999)), parseFormat)
  const { disableStart } = props
  if (disableStart) {
    yearEnd = new Date(yearEnd).getTime() - disableStart.getTime() > 0 ? dateToString(disableStart, parseFormat) : yearEnd
  }
...
function disableDate(date: Date) {
  return props.disableStart ?
    date > props.disableStart : 
    false
}
/**
...
function openDate() {
  ...
  nextTick(() => {
    ...
    cellArray.forEach(c => {
      ...
      const needStop = (type === 'ten' && c.getAttribute('data-index')) || (type === 'quarter' && c.getAttribute('data-month'))
      if (needStop) {
...
function cellClick(e: Event) {
  ...
  if (isTenQuarter.value) {
    ...
    if (type === 'ten') {
      ...
      const fullYear = currentCalendarDate.value.getFullYear()
...
watch(currentCalendarDate, (newVal) => {
  ...
  for (const [index, btn] of btns.entries()) {
    ...
    if (type === 'ten') {
      ...
      if (calMonthStart.getTime() - disableStart.getTime() > 0) { // 当前面板月份大于禁选日期，全部禁选
        setDisable(btn, true)
        continue
      }
    ...
    if (type === 'quarter') {
      ...
      if (calYearStart.getTime() - disableStart.getTime() > 0) { // 当前面板年份大于禁选日期，全部禁选
        setDisable(btn, true)
        continue
      }
