...
function valueDiff(value: string[], oldValue: string[]) {
...
const emits = defineEmits<{
  ...
  (e: "change", date: Date, type: TypeString, otype: string): void;
  (e: "calendar-change", date: Date[]): void;
}>();

// 处理value
// valueWhenOpen：仅针对询和季，打开面板时，暂存已选时间，防止用户在面板中选了其他时间，但没有点确认按钮，datepicker时间不还原的问题
const valueWhenOpen = ref<string[] | null>(null);
// templateValue：对modelValue进行一层隔离，当需要确认时，只在点击确认按钮时，才抛出update事件
...
const innerValue = computed({
  get() {
    ...
    if (innerRange.value) {
      ...
      result = isTenQuarter.value
        ? parseInnerValueToRightFormat(templateValue.value[0], props.type)
        : templateValue.value;
    } else {
      if (props.type === "date" && props.defaultTime) {
        // 如果date类型，设置了默认时分，则输入的时候去掉
        result = dateToString(new Date(templateValue.value[0]), `YYYY-MM-DD 00:00:00`);
      } else {
        result = templateValue.value[0];
      }
    }
    return result;
  },
  set(value) {
    ...
    if (Array.isArray(value)) {
      ...
      // quarterEndNext为true：当type是quarter时，需要将结束时间改为次月1日（组件内部仍然是结束月份最后1天，否则视图将显示次月也被选中）
      if (props.type === "quarter" && value.length === 2 && props.quarterEndNext) {
    ...
    templateValue.value = result;
    // 1.不需要确认时，立即抛出
    // 2.不是询和季时，vue-date-picker内部已经处理了确认事件
    // 3.点击了快捷选项时，不必再确认
    if (!props.confirm || (props.type !== "quarter" && props.type !== "ten") || clickShortCut.value) {
      valueWhenOpen.value = null;
      if (valueDiff(result, props.modelValue)) emits("update:modelValue", result);
    }
    clickShortCut.value = false;
  },
});
watch(
  () => props.modelValue,
...
function getTenRangeByDate(currentDate: Date, startDay?: TenStartNums) {
  ...
  let newDateEnd = new Date(`${fullYear}-${month < 10 ? `0${month}` : month}-${dateEnd} 23:59:59`);
  const { disableStart } = props;
  if (disableStart) {
...
function getQuarterRangeByDate(currentDate: Date, startMonth?: QuarterStartNums) {
  ...
  let newDateEnd = new Date(`${yearEnd}-${monthEnd < 10 ? `0${monthEnd}` : monthEnd}-${monthEndDate} 23:59:59`);
  const { disableStart } = props;
  if (disableStart) {
    newDateEnd = new Date(newDateEnd).getTime() - disableStart.getTime() > 0 ? disableStart : newDateEnd;
  }
 ...
 * 快捷选项
 */
const clickShortCut = ref(false); // 点击了快捷选项时置位，抛出update事件后复位
const tenShortcuts = [
  { text: "上旬", date: 1 },
  { text: "中旬", date: 11 },
  { text: "下旬", date: 21 },
...
function openDate() {
  valueWhenOpen.value = props.modelValue; // 每当弹窗打开时暂存一下当前值
  // nextTick：为确认按钮注册点击事件（仅当存在时），vue-date-picker内部的confirm事件比close触发晚
  // 如果点击了确认，要在close之前将valueWhenOpen.value清空
  nextTick(() => {
    const confirmBtn = datepicker.value?.getElementsByClassName("mx-datepicker-btn-confirm");
    if (confirmBtn && confirmBtn.length > 0) {
      const cbtn = confirmBtn[0];
      cbtn.addEventListener("click", clickConfirm, true); // 事件捕获
    }
  });
...
function cellClick(e: Event) {
  ...
  if (isTenQuarter.value) {
    ...
    if (type === "ten") {
      ...
      // 由于拦截了点击事件，一些原本的事件不会再触发，需要在这里手动触发一下
      emits("pick", new Date(new Date(currentDateString).setMonth(month - 1, cellValue)));
      nextTick(() => {
        innerChange(new Date(new Date(currentDateString).setMonth(month - 1, cellValue)), innerType.value);
      });
      ...
      nextTick(() => {
        innerChange(new Date(new Date(currentDateString).setMonth(cellValue, 1)), innerType.value);
      });
    }
    // 手动触发setter
...
function clickConfirm() {
  valueWhenOpen.value = null;
  if (!props.confirm || (props.type !== "quarter" && props.type !== "ten")) return;
  if (templateValue.value && templateValue.value.length > 0) {
    if (valueDiff(templateValue.value, props.modelValue)) emits("update:modelValue", templateValue.value);
  }
}
// close事件
function closeDate() {
  if (valueWhenOpen.value) {
    if (valueDiff([...valueWhenOpen.value], props.modelValue)) emits("update:modelValue", [...valueWhenOpen.value]);
    valueWhenOpen.value = null;
  }
  const confirmBtn = datepicker.value?.getElementsByClassName("mx-datepicker-btn-confirm");
  if (confirmBtn && confirmBtn.length > 0) {
    const cbtn = confirmBtn[0];
    cbtn.removeEventListener("click", clickConfirm);
  }
}
// change事件
function innerChange(date: Date, type: string) {
  emits("change", date, props.type, type);
}
...
<template>
  <VueDatePicker
    ...
    @calendar-change="(calendarChange as any)"
    @pick="(pickDate as any)"
    @open="openDate"
    @close="closeDate"
    @change="(innerChange as any)"
  >
...
.popup-hideone {
  .mx-calendar-range {
    ...
    .mx-table-month {
      .cell {
        // 解除注释可控制选中旬、季时间段两端的单元格是浅蓝色，与默认的周选择功能一致
