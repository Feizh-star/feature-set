...
const MapMask = L.Layer.extend({
  ...
  resetLayerPosition() {
    ...
    const new_position = this._map.latLngToLayerPoint([
  ...
  zoomEvent: function () {
    ...
    const offset = L.Layer
      ...
      : map.getCenterOffset(center).multiplyBy(-scale).subtract(map.getMapPanePos())
    L.DomUtil.setTransform(this.canvas, offset, scale)
  },
  // 缩放、拖动、resize之后都会触发moveend事件
  moveEvent: function () {
    this.resetLayerPosition()
    this.draw()
  },
