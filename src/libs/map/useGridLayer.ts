...
export function useGridLayer({ mapIns }: { mapIns: ShallowRef<L.Map | null> }) {
  ...
  const setGridLayerData = (
    gridUrl: string,
    dirUrlOrOptions: string | null,
    options: { [p: string]: any } = {}
  ) => setGridLayerDataFunc(mapIns, gridLayer, gridUrl, dirUrlOrOptions, options)
  const updateGridImgUrl = (url: string, dirUrl?: string) =>
    updateGridImgUrlFunc(gridLayer, url, dirUrl)
